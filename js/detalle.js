...
var longitude = "";
var check = true;
var dominio = "http://ontrack.tmeridian.com.pe:9002/api/";
var dominio_extranet = "http://ontrack.tmeridian.com.pe:9002/";
var dominio_foto = "http://ontrack.tmeridian.com.pe:9002/";
...
$(document).ready(function(e) {
 ...
 $("#registrarIncidencia").click(function(e) {
	...
	$.mobile.loading('show'); 
	$.ajax({
	    url: dominio + "Operaciones/Chofer.asmx/GenerarTrakingV2",
        type: "POST",
		//crossDomain: true,
        dataType : "json",
	...
	if ( parametros.IDEstado == 7){
		...
		$("input[id*='chkProdcuto']").each(function(index, element) {
            ...
            $.ajax({
                url: dominio + "Operaciones/Chofer.asmx/TrackinkDetalle",
				type: "POST",
				dataType : "json",
		...
		if ( $.QueryString["grupo"] == 1 ){
			...
			if ( index == (totalOC-1)){ //Es el ultimo
				...
				$.ajax({
				    url: dominio + "Operaciones/Chofer.asmx/GenerarTrakingGrupo",
					type: "POST",
					dataType : "json",
					data : JSON.stringify(parametros),
					contentType: "application/json; charset=utf-8",
				...
				$.ajax({
				    url: dominio + "Operaciones/Chofer.asmx/GenerarTrakingGrupo_Simple",
					type: "POST",
					dataType : "json",
					data : JSON.stringify(parametros),
					contentType: "application/json; charset=utf-8",
			...
			$.ajax({
			    url: dominio + "Operaciones/Chofer.asmx/GenerarTrakingV3",
				type: "POST",
				dataType : "json",
				data : JSON.stringify(parametros),
				contentType: "application/json; charset=utf-8",
...
function HabilitarIncidencia(control){
 ...
 if ( $(control).val() == 5 ){
     $(".contentAtencion1, #DIVIncidencia").show();
 }
 else if ($(control).val() == 6) {
     $(".contentDatos, .contentAtencion1").show();
 }
 else if ( $(control).val() == 4 ){
     $("#btnIncidencia").show("fast");
     $(".contentAtencion1").show();
 }
 else if (  $(control).val() == 7 ){
     $("#DIVIncidencia, #panelParcial, .contentDatos, .contentAtencion1").show();
 }
 else{
...
function setTracking(idPedido){
	...
	$.mobile.loading('show'); 
	$.ajax({
	    url: dominio + "Operaciones/Chofer.asmx/ObtenerTraking",
        type: "POST",
		cache: false,
		//crossDomain: true,
        dataType : "json",
...
function setIncidencias_Tracking(empresa, idestado){
	...
	//$.mobile.loading('show'); 
	$.ajax({
	    url: dominio + "Operaciones/Chofer.asmx/Obtener_IncidenciaPorEstado",
        type: "POST",
		cache: false,
		//crossDomain: true,
        dataType : "json",
...
function setPedido(idPedido){
	...
	$.mobile.loading('show'); 
	$.ajax({
	    url: dominio + "Operaciones/Chofer.asmx/ObtenerPedido",
        type: "POST",
		//crossDomain: true,
        dataType : "json",
        data : '{"IDPedido":"'+idPedido+'"}',
...
function setDetallePedido(idPedido){
	...
	$.mobile.loading('show'); 
	$.ajax({
	    url: dominio + "Operaciones/Chofer.asmx/ObtenerDetallePedido",
        type: "POST",
		//crossDomain: true,
        dataType : "json",
        data : '{"IDPedido":"'+idPedido+'"}',
...
function setDetallePedidoGrupo(grupo,chofer,entidad){
	...
	//
	$.ajax({
	    url: dominio + "Operaciones/Chofer.asmx/ConsultarPedidosGrupo",
        type: "POST",
		//crossDomain: true,
        dataType : "json",
...
function setFotosPedido(idPedido){
	...
	$.mobile.loading('show'); 
	$.ajax({
	    url: dominio + "Operaciones/Chofer.asmx/ConsultarFotos",
        type: "POST",
		//crossDomain: true,
        dataType : "json",
        data : '{"IDPedido":"'+idPedido+'"}',
        ...
        success : function(data, textStatus, jqXHR) {
			...
			if ( resultado.length > 0 ){
				...
				for (var i = 0; i<resultado.length;i++){
				    ...
				    html += "<div class='imgPanel'><div style='background-image:url(" + resultado[i].Ubicacion.replace("~", dominio_foto) + ");height: 180px;background-position: top center;background-size: cover;'></div> <a onclick='quitarFoto(" + resultado[i].IDFoto + ", this)'>Borrar</a></div>";
				    //if ( (i%2)!=0 && i>0 )
						//html += "</tr><tr>"; 			 
				}
				//html += "</tr></table>";		
